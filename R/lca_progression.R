...
#'
#' @param glob_to_pid A character string that specifies the file naming convention for each zotu table with different lca parameters up to the percent identity (pid) value (which needs to be in the file name)
...
#' @returns A data frame with one row for every zotu identified at any pid level with 11 columns including:
#' * taxonomic information across 8 levels in descending order (domain, kingdom, phylum, class, order, family, genus, species)
#' * zotu name (named "zotu")
#' * the number of unique blast hits (numberOfUnq_BlastHits)
#' * the highest percent identity level at which the zotu was classified (pid)
#' @examples
#' lca_progression()
#' lca_progression(glob_to_pid = "taxonomy_", dir = "~/Desktop/results", lulu = T)
#' @description Merges a series of similarly named zotu tables generated from running the eDNAFlow LCA taxonomy assignment process on the same dataset with different minimum percent identity (pid) thresholds to keep only the highest level match per zotu. The main point of this is to provide the approximate pid match for each zotu that can be examined alongside the number of unique blast hits to determine whether there may be a dubious assignment from unclear, incomplete, or incorrect database listings.
#' @export
